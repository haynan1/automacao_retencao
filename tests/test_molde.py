...
def test_setor_com_nome_de_tipo_e_aceito_ate_sem_linha_total():
    """O bloco passou a ser delimitado por ESTRUTURA, não por vocabulário.

    Antes, um setor chamado "UBS 13 DE MAIO" só era aceito com a linha TOTAL
    ligada: sem ela, a varredura confundia o nome com uma linha de tipo e
    engolia o bloco seguinte. Agora o que separa um do outro é a linha
    'Tipo' logo abaixo do nome do setor — e ela está sempre lá.
    """
    for linha_total in (True, False):
        opcoes = dict(spec_valida()["opcoes"], linha_total_bloco=linha_total)
        spec, wb, divergencias = molde.construir(
            spec_valida(setores=[{"nome": "UBS 13 DE MAIO"}, {"nome": "MENSAL SUL"}],
                        opcoes=opcoes)
        )
        assert divergencias == []
        blocos = preenchimento.localizar_blocos_setores(wb["MOLDE"])
        assert [b["setor"] for b in blocos] == ["UBS 13 DE MAIO", "MENSAL SUL"]
        assert all(b["tipos"] == spec["tipos"] for b in blocos)


def test_setor_com_total_no_nome_e_recusado():
    """O motor fecha o bloco em qualquer rótulo com TOTAL — inclusive no meio."""
    assert any("TOTAL" in p for p in problemas_de(setores=[{"nome": "SUBTOTAL NORTE"}]))
...
def test_listas_vazias_sao_recusadas():
    ...
    assert any("ao menos uma linha de tipo" in p for p in problemas_de(tipos=[]))


def test_tipo_livre_e_aceito():
    """Qualquer rótulo é uma linha válida — o motor lê o que estiver lá.

    Enquanto a lista era fechada, desenhar uma linha de FÉRIAS era proibido
    e as férias só tinham como somar na mensal.
    """
    spec = molde.validar_spec(spec_valida(tipos=["Mensal", "Rescisão", "Férias 1/3"]))
    assert spec["tipos"] == ["Mensal", "Rescisão", "Férias 1/3"]


def test_tipo_repetido_a_menos_de_acento_e_recusado():
    problemas = problemas_de(tipos=["13º salário", "13o SALARIO"])
    assert any("repete" in p for p in problemas)


def test_tipo_reservado_e_recusado():
    """O motor fecha o bloco em TOTAL e usa 'Tipo' como cabeçalho."""
    assert any("fecha o bloco" in p for p in problemas_de(tipos=["Mensal", "TOTAL"]))
    assert any("cabeçalho" in p for p in problemas_de(tipos=["Tipo"]))


def test_teto_de_tipos_cabe_na_janela_do_motor():
    """Guarda de contrato entre construtor e motor.

    Se o construtor aceitasse mais linhas do que a varredura percorre, ele
    gravaria um molde que o preenchimento não consegue ler inteiro.
    """
    assert molde.MAX_TIPOS <= preenchimento.MAX_LINHAS_TIPO
...
def test_gera_e_o_motor_le_exatamente_o_desenhado():
    ...
    for bloco in blocos:
        assert bloco["tipos"] == ["Mensal", "13º salário"]
...
def test_linha_total_e_formula_e_nunca_recebe_valor():
    ...
    linha_mensal = bloco["linhas_tipo"][preenchimento.chave_tipo("Mensal")]
...
def test_estilos_em_cache_nao_vazam_entre_workbooks(tmp_path):
    ...
    for caminho in caminhos:
        ...
        valor = ws.cell(row=bloco["linhas_tipo"][preenchimento.chave_tipo("Mensal")], column=2)
...
def test_molde_construido_processa_de_verdade(listagem, tmp_path):
    ...
    mapeador.resolver_tipos(lancamentos, tipos, {})
    ...
    escrito = ws.cell(row=acs["linhas_tipo"][preenchimento.chave_tipo("Mensal")], column=coluna).value
