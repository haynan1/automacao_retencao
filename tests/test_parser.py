...
def test_valor_e_folha(listagem):
    ...
    # O parser NÃO decide destino: ele preserva a folha e a classifica.
    # Decidir aqui era o que apagava FÉRIAS antes de qualquer tela ver.
    ferias = next(l for l in r["lancamentos"] if l["folha"] == "FÉRIAS")
    assert ferias["folha_familia"] == "Férias"
    assert ferias["tipo_destino"] is None
    assert "FÉRIAS" in r["folhas"] and "MENSAL" in r["folhas"]
