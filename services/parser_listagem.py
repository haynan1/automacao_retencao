...
from .normalizador import familia_folha, normalizar_texto
...
def extrair_lancamentos(caminho_xlsx: str | Path) -> dict:
    ...
    for r in range(1, ws.max_row + 1):
        ...
        # O parser NAO decide o destino da folha. Ele registra o que leu e a
        # familia a que o texto pertence; a linha de destino sai do vinculo
        # do perfil (mapeador.resolver_tipos), confirmavel na tela.
        registro = {
            ...
            "folha": folha_bruta,
            "folha_familia": familia_folha(folha_bruta),
            "tipo_destino": None,
            "folha_status": None,
    ...
    for reg in lancamentos:
        ...
        if lot and lot not in lotacoes:
            lotacoes.append(lot)
        # As folhas distintas do relatorio alimentam o eixo folha -> linha:
        # a tela precisa mostrar TODAS, inclusive as que nao tem destino.
        if reg["folha"] and reg["folha"] not in folhas:
            folhas.append(reg["folha"])
