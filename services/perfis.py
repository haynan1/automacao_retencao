...
os proprios vinculos aprendidos dos tres eixos. O MOTOR e 100% reaproveitado
entre secretarias — o que muda sao apenas os DADOS de cada perfil, evitando
colisao de vinculos entre secretarias.

Layout em disco:
    config/perfis.json                                   (registro de perfis)
    config/perfis/<slug>/mapeamento_lotacoes.json        (eixo 1: lotacao -> setor)
    config/perfis/<slug>/vinculo_rubrica_coluna.json     (eixo 2: evento -> coluna)
    config/perfis/<slug>/vinculo_folha_tipo.json         (eixo 3: folha -> linha)
    modelos/perfis/<slug>/molde_padrao.xlsx
    modelos/perfis/<slug>/molde_padrao.json
    modelos/perfis/<slug>/molde_estrutura.json           (o desenho do molde)
...
from .utils import CONFIG_DIR, MODELOS_DIR, criar_pastas, gravar_json
...
def _salvar_registro(reg: dict) -> None:
    """Escrita atomica: perder o registro apagaria TODAS as secretarias."""
    criar_pastas()
...
def caminho_vinculos_folhas(slug: str) -> Path:
    """Vinculo folha -> linha de tipo. Isolado por perfil pelo mesmo motivo
    dos demais: o 'FÉRIAS' de uma secretaria pode ter linha propria e o de
    outra nao, e um vazamento entre perfis moveria dinheiro de lugar."""
    return _dir_config(slug) / "vinculo_folha_tipo.json"
