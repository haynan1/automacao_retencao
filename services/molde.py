...
from .utils import gravar_json

log = logging.getLogger("automacao_retencao")

SPEC_VERSAO = 1

# Linhas de tipo sugeridas na interface — atalhos, nao um limite. O motor
# le da planilha o rotulo que estiver la, entao qualquer nome e um destino
# valido: quem tem uma linha propria de FÉRIAS desenha FÉRIAS e para de
# somar ferias na mensal.
TIPOS_SUGERIDOS = ("Mensal", "13º salário", "Férias", "Rescisão", "Complementar")
# Com o que uma folha em branco comeca (o arranjo classico da secretaria).
TIPOS_PADRAO = ("Mensal", "13º salário")
...
MAX_SETORES = 200
MAX_COLUNAS = 200
# Teto de linhas de tipo por bloco. Precisa caber na janela de varredura do
# motor (`preenchimento.MAX_LINHAS_TIPO`), senao o construtor aceitaria um
# molde que o preenchimento nao consegue ler inteiro.
MAX_TIPOS = 40
...
def _validar_setores(bruto, problemas: list[str]) -> list[dict]:
    ...
    for item in itens:
        ...
        if norm == "TIPO":
            ...
            continue
        # O motor fecha o bloco em QUALQUER rotulo que contenha TOTAL — um
        # setor assim seria lido como a linha de total do bloco anterior e
        # levaria o bloco seguinte junto. Recusa aqui o que ele descartaria.
        if "TOTAL" in norm:
            problemas.append(
                f"{rotulo}: nomes com “TOTAL” se confundem com a linha de total "
...
def _validar_tipos(bruto, problemas: list[str]) -> list[str]:
    """Linhas de tipo: lista livre, validada pelas mesmas regras das colunas.

    O motor le o rotulo que estiver na planilha, entao aqui nao ha lista
    fechada — ha as recusas do que ele descartaria: um rotulo iniciado por
    TOTAL fecharia o bloco, “Tipo” e o cabecalho, e um duplicado deixaria a
    segunda linha para sempre vazia.
    """
    itens = _itens_examinaveis(bruto, "Linhas de tipo de folha", problemas)
    if itens is None:
        return []
    ...
    for item in itens:
        ...
        norm = normalizar_texto(nome)
        if norm == "TIPO":
            problemas.append(f"{rotulo}: “Tipo” é o rótulo do cabeçalho do bloco.")
            continue
        if "TOTAL" in norm:
            problemas.append(
                f"{rotulo}: “TOTAL” fecha o bloco — uma linha com esse nome "
                "encerraria a leitura e nunca receberia lançamento."
            )
            continue
        if norm in vistos:
            problemas.append(
                f"{rotulo}: repete “{vistos[norm]}” (acentos e caixa não diferenciam) — "
                "a segunda linha nunca seria preenchida."
            )
            continue
        vistos[norm] = nome
        tipos.append(nome)
    ...
    if not tipos:
        problemas.append(
            "Defina ao menos uma linha de tipo de folha (Mensal, 13º salário, Férias…) — "
            "é a linha onde o lançamento cai dentro do bloco."
        )
    elif len(tipos) > MAX_TIPOS:
        problemas.append(
            f"Máximo de {MAX_TIPOS} linhas de tipo por bloco (recebidas {len(tipos)})."
        )
    return tipos
...
def validar_spec(bruto: dict) -> dict:
    ...
    spec = {
        ...
        "setores": _validar_setores(bruto.get("setores"), problemas),
...
def verificar_workbook(wb, spec: dict) -> list[str]:
    ...
    for nome in spec["abas"]:
        ...
        for bloco in blocos:
            if bloco["tipos"] != spec["tipos"]:
                divergencias.append(
                    f"Aba “{nome}”, setor “{bloco['setor']}”: o motor leu "
                    f"{len(bloco['tipos'])} linha(s) de tipo contra {len(spec['tipos'])} "
                    f"desenhada(s). Diferença em "
                    f"“{_primeira_diferenca(bloco['tipos'], spec['tipos'])}”."
                )
...
def extrair_spec(caminho: str | Path) -> dict:
    ...
    try:
        ...
        # As linhas de tipo saem da propria planilha, com o rotulo que ela
        # usa. Antes so voltavam as duas conhecidas — reabrir um molde com
        # uma linha de FÉRIAS a apagava silenciosamente no primeiro salvar.
        ...
        return validar_spec({
            ...
            "colunas": colunas,
            "setores": setores,
            "tipos": tipos or list(TIPOS_PADRAO),
            "opcoes": {
...
def salvar_estrutura(slug: str, spec: dict) -> None:
    ...
    Escrita atomica (ver `utils.gravar_json`): uma queda no meio da escrita
    deixaria um JSON pela metade — e o desenho de dezenas de setores voltaria
    como 'folha em branco' na proxima abertura.
    """
    gravar_json(caminho_estrutura(slug), spec)
